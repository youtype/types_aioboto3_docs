...
async with session.client("voice-id") as client:  # (1)
    kwargs: AssociateFraudsterRequestRequestTypeDef = {...}  # (2)
    result: AssociateFraudsterResponseTypeDef = await client.associate_fraudster(**kwargs)  # (3)
```

1. client: [VoiceIDClient](./client.md)
2. kwargs: [:material-code-braces: AssociateFraudsterRequestRequestTypeDef](./type_defs.md#associatefraudsterrequestrequesttypedef) 
