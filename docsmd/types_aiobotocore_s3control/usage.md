...
async with session.client("s3control") as client:  # (1)
    kwargs: AssociateAccessGrantsIdentityCenterRequestRequestTypeDef = {...}  # (2)
    result: EmptyResponseMetadataTypeDef = await client.associate_access_grants_identity_center(**kwargs)  # (3)
```

1. client: [S3ControlClient](./client.md)
2. kwargs: [:material-code-braces: AssociateAccessGrantsIdentityCenterRequestRequestTypeDef](./type_defs.md#associateaccessgrantsidentitycenterrequestrequesttypedef) 
