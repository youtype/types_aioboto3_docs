...
async with session.client("bedrock") as client:  # (1)
    kwargs: BatchDeleteEvaluationJobRequestRequestTypeDef = {...}  # (2)
    result: BatchDeleteEvaluationJobResponseTypeDef = await client.batch_delete_evaluation_job(**kwargs)  # (3)
```

1. client: [BedrockClient](./client.md)
2. kwargs: [:material-code-braces: BatchDeleteEvaluationJobRequestRequestTypeDef](./type_defs.md#batchdeleteevaluationjobrequestrequesttypedef) 
