...
async with session.client("datasync") as client:  # (1)
    kwargs: AddStorageSystemRequestRequestTypeDef = {...}  # (2)
    result: AddStorageSystemResponseTypeDef = await client.add_storage_system(**kwargs)  # (3)
```

1. client: [DataSyncClient](./client.md)
2. kwargs: [:material-code-braces: AddStorageSystemRequestRequestTypeDef](./type_defs.md#addstoragesystemrequestrequesttypedef) 
