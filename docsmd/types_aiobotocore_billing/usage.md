...
async with session.client("billing") as client:  # (1)
    kwargs: CreateBillingViewRequestRequestTypeDef = {...}  # (2)
    result: CreateBillingViewResponseTypeDef = await client.create_billing_view(**kwargs)  # (3)
```

1. client: [BillingClient](./client.md)
2. kwargs: [:material-code-braces: CreateBillingViewRequestRequestTypeDef](./type_defs.md#createbillingviewrequestrequesttypedef) 
