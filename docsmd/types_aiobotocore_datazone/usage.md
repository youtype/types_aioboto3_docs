...
async with session.client("datazone") as client:  # (1)
    paginator = client.get_paginator("list_asset_revisions")  # (2)
...
async with session.client("datazone") as client:  # (1)
    kwargs: AcceptPredictionsInputRequestTypeDef = {...}  # (2)
    result: AcceptPredictionsOutputTypeDef = await client.accept_predictions(**kwargs)  # (3)
```

1. client: [DataZoneClient](./client.md)
2. kwargs: [:material-code-braces: AcceptPredictionsInputRequestTypeDef](./type_defs.md#acceptpredictionsinputrequesttypedef) 
