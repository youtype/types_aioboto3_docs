...
async with session.client("mailmanager") as client:  # (1)
    kwargs: CreateAddonInstanceRequestRequestTypeDef = {...}  # (2)
    result: CreateAddonInstanceResponseTypeDef = await client.create_addon_instance(**kwargs)  # (3)
```

1. client: [MailManagerClient](./client.md)
2. kwargs: [:material-code-braces: CreateAddonInstanceRequestRequestTypeDef](./type_defs.md#createaddoninstancerequestrequesttypedef) 
