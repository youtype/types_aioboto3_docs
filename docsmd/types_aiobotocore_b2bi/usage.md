...
client: B2BIClient
async with session.client("b2bi") as client:  # (1)
    kwargs: CreateCapabilityRequestRequestTypeDef = {...}  # (2)
    result: CreateCapabilityResponseTypeDef = await client.create_capability(**kwargs)  # (3)
```

1. client: [B2BIClient](./client.md)
2. kwargs: [:material-code-braces: CreateCapabilityRequestRequestTypeDef](./type_defs.md#createcapabilityrequestrequesttypedef) 
