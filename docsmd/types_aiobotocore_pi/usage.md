...
client: PIClient
async with session.client("pi") as client:  # (1)
    kwargs: CreatePerformanceAnalysisReportRequestRequestTypeDef = {...}  # (2)
    result: CreatePerformanceAnalysisReportResponseTypeDef = await client.create_performance_analysis_report(**kwargs)  # (3)
```

1. client: [PIClient](./client.md)
2. kwargs: [:material-code-braces: CreatePerformanceAnalysisReportRequestRequestTypeDef](./type_defs.md#createperformanceanalysisreportrequestrequesttypedef) 
